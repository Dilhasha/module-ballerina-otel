...
import io.ballerina.observe.trace.otel.logging.OtelTraceLogger;
...
public final class OtelExporter implements SpanExporter {
    private final String endpoint;
    private final SpanExporter exporter;
    private final OtelTraceLogger tracerLogger;

    public OtelExporter(SpanExporter exporter, String endpoint,
        ...
        if (traceLogConsole || !traceLogFile.isEmpty()) {
            ...
            this.tracerLogger = new OtelTraceLogger(traceLogConsole, logFilePath);
            this.tracerLogger.setLogLevel(getTraceLogLevel(traceLogLevel));
        } else {
            this.tracerLogger = new OtelTraceLogger();
    ...
    public CompletableResultCode export(Collection<SpanData> spans) {
        ...
        result.whenComplete(() -> {
            if (result.isSuccess()) {
                if (tracerLogger.isLoggable(Level.INFO)) {
                    tracerLogger.printInfo("✓ Successfully exported " + spans.size() + " spans to " + endpoint);
                }
            } else {
                if (tracerLogger.isLoggable(Level.SEVERE)) {
                    tracerLogger.printSevere("✗ FAILED to export " + spans.size() + " spans to " + endpoint);
                    tracerLogger.printSevere("Export result: " + result);
                }
            }
        });
        return result;
